...
hpke1_example(void)
{
    ...
    struct t_cose_key                skR;
    struct t_cose_key                pkR;
    Q_USEFUL_BUF_MAKE_STACK_UB  (    cose_encrypt_message_buffer, 2000);
    Q_USEFUL_BUF_MAKE_STACK_UB  (    decrypted_plaintext_buffer, 500);
    struct t_cose_recipient_dec_hpke dec_recipient;
    struct t_cose_encrypt_dec_ctx    dec_ctx;

    printf("\n---- START EXAMPLE HPKE ----\n");
    printf("Create COSE_Encrypt with included payload using HPKE with KEM: DHKEM(P-384, HKDF-SHA384), KDF: HKDF-SHA384, AEAD: AES-256-GCM\n");

    /* Create a key pair.  This is a fixed test key pair. The creation
     * of this key pair is crypto-library dependent because t_cose_key
     * is crypto-library dependent. See t_cose_key.h and the examples
     * to understand key-pair creation better. */
    result = init_fixed_test_ec_encryption_key(T_COSE_ELLIPTIC_CURVE_P_384,
                                               &pkR, /* out: public key to be used for encryption */
                                               &skR); /* out: corresponding private key for decryption */
    if(result != T_COSE_SUCCESS) {
        goto Done;
...
hpke2_example(void)
{
    ...
    printf("\n---- START EXAMPLE HPKE ----\n");
    printf("Create COSE_Encrypt with included payload using HPKE with KEM: DHKEM(P-521, HKDF-SHA512), KDF: HKDF-SHA512, AEAD: AES-256-GCM\n");

    /* Create a key pair.  This is a fixed test key pair. The creation
     * of this key pair is crypto-library dependent because t_cose_key
     * is crypto-library dependent. See t_cose_key.h and the examples
     * to understand key-pair creation better. */
    result = init_fixed_test_ec_encryption_key(T_COSE_ELLIPTIC_CURVE_P_521,
                                               &pkR, /* out: public key to be used for encryption */
                                               &skR); /* out: corresponding private key for decryption */
    if(result != T_COSE_SUCCESS) {
        goto Done;
     ...
     */
    t_cose_recipient_enc_hpke_init(&recipient,
                                    T_COSE_HPKE_Base_P521_SHA512_AES256GCM);
